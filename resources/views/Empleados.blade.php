    ...
    <div class="px-8 py-24 mx-auto md:px-12 lg:px-32 max-w-7xl">
      <div class="text-center">
        <h1 class="text-4xl font-semibold tracking-tighter text-gray-900 lg:text-5xl text-balance">
          ...
          <span class="text-gray-600">Haciendo las Consultas más fácil y seguras</span>
        </h1>
        <p class="w-1/2 mx-auto mb-4 mt-4 text-base font-medium text-gray-500 text-balance">
         Sección para Agregar, Modificar y Eliminar Empleados
        </p>
      </div>
      <form action="{{ isset($empleado) ? route('empleados.update', $empleado->id) : route('empleados.store') }}" method="POST">
        ...
        <div class="mb-4 flex space-x-4">
          <div class="w-1/2">
            ...
            <input type="text" id="device-name" name="nombre" value="{{ $empleado->EMP_NOMBRE ?? '' }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
          </div>
          <div class="w-1/2">
            <label for="device-name" class="block text-sm font-medium text-gray-700">DNI</label>
            <input type="text" id="device-name" name="dni" value="{{ $empleado->EMP_DNI ?? '' }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
          </div>
        </div>
        ...
        <div class="mb-4 flex space-x-4">
          ...
          <div class="w-1/2">
            ...
            <select name="estado" id="role" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
              <option value="" disabled selected>Selecciona el estado</option>
              <option value="1" {{ isset($empleado) && $empleado->EMP_ESTADO == 1 ? 'selected' : '' }}>Activo</option>
              <option value="0" {{ isset($empleado) && $empleado->EMP_ESTADO == 0 ? 'selected' : '' }}>Inactivo</option>
            </select>
          </div>
        </div>
        ...
        <div class="mb-4 flex space-x-4">
          <div class="w-1/2">
            ...
            <input type="text" id="device-name" name="departamento" value="{{ $empleado->EMP_DEPARTAMENTO ?? '' }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
          </div>
          <div class="w-1/2">
            <label for="device-name" class="block text-sm font-medium text-gray-700">RFID</label>
            <input type="text" id="device-name" name="rfid" value="{{ $empleado->EMP_RFID ?? '' }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
          </div>
        </div>
